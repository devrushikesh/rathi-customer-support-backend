import type { 
  ...
  class CustomerServices {
    ...
    static async getIssueStats(customerId: number) {
      try {
        const [totalCount, activeCount, closedCount, criticalCount] = await Promise.all([
          prisma.issue.count({
            where: { customerId }
          }),
          prisma.issue.count({
  ...
  // 1. Create an issue
  const newIssue = await CustomerServices.createIssue({
    description: "Machine not starting properly",
    machine: "CNC-001",
    priority: "HIGH",
  ...
  // 2. Get all active issues
  const activeIssues = await CustomerServices.fetchActiveIssues(123);
  
  // 3. Get all closed issues
  const closedIssues = await CustomerServices.fetchClosedIssues(123);
  
  // 4. Get all issues (active + closed)
  const allIssues = await CustomerServices.fetchAllIssues(123);
  
  // 5. Get timeline for specific issue
  const timeline = await CustomerServices.fetchIssueTimeline("issue_id_here", 123);
  
  // 6. Get issue statistics
  const stats = await CustomerServices.getIssueStats(123);
  
  // 7. Search issues with filters
  const searchResults = await CustomerServices.searchIssues(123, {
    query: "machine not working",
  ...
  // 8. Get single issue by ID
  const singleIssue = await CustomerServices.getIssueById("issue_id_here", 123);
